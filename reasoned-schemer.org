...
* 3:14
  ...
  (From [[https://github.com/hoenigmann/reasoned-schemer/blob/master/reasoned-schemer.org][Albert]]: is the $n^{th}$ iteration of =run= aware of the
  $n-1^{st}$'s result? In other words, is memoization taking place; or
  does the calculation start from scratch every time?)
* Footnotes
