...
** Reader macros for =#s= and =#u=
   ...
   See [[http://wiki.call-cc.org/man/4/faq#why-does-define-reader-ctornot-work-in-my-compiled-program][this]].
** "Why?" vs. "how?"
...
* 3:65
  ...
  #+END_SRC
* 3:66
  =e= is associated with $x$, since it is the only value that fulfills
  =membero=.
...
* 3:67
  ...
  "Because =(membero e (pasta e fagioli))= succeeds," mention the
  authors.
* 3:68
  Inverted the relationship between question and answer.
* 3:69
  $x$ remains fresh because =(membero e (pasta e ...))= succeeds. On
  the other hand, =((eq-caro l x) #s)= is wrapped in a =conde=; which
  implies that $x$ becomes =e= when =conde= fails.
  ...
  Nope: my first hypothesis was correct; does that mean that recursive
  =membero= is not invoked? Yes, because it's =(run 1 ...)= not =(run*
  ...)=; in the latter case, $x$ might have been =e=.
  ...
  Nope: neither! It's both fresh and =e=, since they both satisfy the
  =membero=-constraint.

  The authors mention that, "the recursion succeeds before getting to
  $x$:" succinct.
* 3:70
  =e= is associated with $x$, since it's the first value that
  succeeds.
  ...
  The authors mention that, "the recursion succeeds when it gets to
  $x$:" nice.
* 3:71
  I suspect that =(e _.0)= and =(_.0 e)= fulfill =membero=: the first
  when the recursion hits $x$ and not yet $y$; the second after $x$ is
  refreshed and the recursion hits $y$. =((e _.0) (_.0 e))=,
  therefore.
...
* 3:72
  See [[3:71][above]].
* 3:73
  =(tofu . _.0)= is associated with $l$, since any pair will do
  (including a proper list with one element).[fn:4] This is because
  =(eq-caro l x)= succeeds ($l$ being fresh) and associates $x$ with
  the car of $l$.
  ...
  This yields the value =((tofu . _.0))=.
* 3:74
  See [[3:73][above]]: any pair, including a monadic list.

  No, this is not true: any list whose =car= is =tofu=; i.e.,
  infinitely many.
* 3:75
  An infinite number of lists starting with =(tofu ...)=.
* 3:76
  The first five possible lists beginning with =tofu=. Here we're
  getting into breadth- vs. depth-firstness: if depth first, =tofu=
  begins the list; if breadth first, =tofu= migrates endward.
  ...
  What happens is that, after =(eq-caro l x)= establishes =(tofu .
  _.0)=, =(cdro l d)= keep pushing fresh variables in front of it;
  this is depth-first. That's the mechanical explanation; not sure
  what the intuitive explanation is for this ordering: possibly that
  it builds its lists in =cons=-like order, given =cdro=.

  Also, not sure what the breadth-first alternative looks like:
  infinite lists, with endward =tofu=? Yes, I think so.
* 3:77
  See [[3:76][above]].

  According to the authors, =cdro= is responsible for growing the
  list; but =eq-caro= is responsible for placing =tofu= at the end.
* 3:78
  Yes, in the sense that null fulfills the fresh-constraint.
* 3:79
  Null, since we're dealing with proper lists.
* 3:80
  Since it associates the =cdr= of $l$ with null, we should get proper
  lists.

  #+BEGIN_SRC scheme :tangle pmembero.scm
    (define pmembero
      (lambda (x l)
        (conde
         ((nullo l) #u)
  ...
  #+END_SRC
* 3:81
  Saying: =tofu= is a member of a proper list; "proper" is the "p" in
  =pmembero=.

  Therefore, =(#t)= is the value.
  ...
  #+END_SRC
* 3:82
  I was going to say [[3:81][vide supra]], but not for the reason I thought; it
  should have been =(#t #t)=, but we're indeed searching for =tofu= at
  the end of the list.
* Footnotes
